...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <bitcointrustaddress> <amount> [comment] [comment-to]\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <bitcointrustaddress> [minconf=1]\n"
            "Returns the total amount received by <bitcointrustaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <tobitcointrustaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("bitcointrust-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("bitcointrust-lock-wa");
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <bitcointrustaddress>\n"
            "Return information about <bitcointrustaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <bitcointrustpubkey>\n"
            "Return information about <bitcointrustpubkey>.");
